# DingTalk 集成经验

**Type**: long_term
**Source**: opencode_experience
**Created**: 2026-02-14T23:33:39.882269+00:00
**ID**: 125

**Tags**: dingtalk, api, config

---

## 概述
文档时间范围：2026-01-30 至 2026-02-13
系统：Clawdbot AI助手
集成目标：通过钉钉自定义机器人发送消息和图片

### 失败1：图片显示为损坏图标
**问题描述**
- 图片上传成功
- 消息发送成功
- 在钉钉中图片显示为损坏的文件图标，而不是实际图片

**失败原因**
1. **错误的API端点**：
   - 使用了新版API端点：`https://api.dingtalk.com/v1.0/media/upload`
   - 应该使用旧版端点：`https://oapi.dingtalk.com/media/upload`

2. **错误的消息类型**：
   - 使用了：`msgtype: "image"`
   - 应该使用：`msgtype: "sampleImageMsg"`

3. **错误的参数字段**：
   - 使用了：`{"imageKey": media_id}`
   - 应该使用：`{"photoURL": media_id}`

**错误日志**
```
错误信息: gateway closed (1008): unauthorized
API端点使用不当
消息格式不匹配钉钉要求
```

**解决方案**
```javascript
// 正确的图片发送方式
// 1. 上传图片到钉钉服务器（使用旧版API）
const uploadUrl = `https://oapi.dingtalk.com/media/upload?access_token=${accessToken}&type=image`;
const formData = new FormData();
formData.append('media', fs.createReadStream(imagePath));

const uploadResponse = await axios.post(uploadUrl, formData, {
  headers: formData.getHeaders()
});

if (uploadResponse.data.errcode === 0) {
  const media_id = uploadResponse.data.media_id;
  
  // 2. 发送图片消息（使用正确的 msgtype 和参数）
  const msg = {
    msgtype: "sampleImageMsg",  // 关键：使用 sampleImageMsg
    sampleImageMsg: {
      photoURL: media_id  // 关键：使用 photoURL 而非 imageKey
    }
  };
  
  await sendDingTalkMessage(webhookUrl, msg);
}
```

**关键要点**
- ✅ 必须使用旧版API端点 `oapi.dingtalk.com`
- ✅ 消息类型必须是 `sampleImageMsg`
- ✅ 参数字段必须是 `photoURL`，不能是 `imageKey`
- ✅ 官方文档中的参数名称必须完全匹配

---

### 失败2：网关令牌不匹配
**问题描述**
- 浏览器功能无法使用
- CLI客户端连接被拒绝
- 错误信息：`gateway closed (1008): unauthorized: gateway token mismatch`

**失败原因**
1. **令牌配置分散在多个位置**：
   - `gateway.auth.token` - 认证令牌
   - `gateway.remote.token` - 远程令牌
   - 环境变量 `CLAWDBOT_GATEWAY_TOKEN`
   - 这些位置配置不一致

2. **systemd服务文件中的令牌值错误**：
   - systemd服务文件中设置的令牌值与配置文件中不一致
   - 导致CLI客户端和服务端使用不同的令牌

**错误日志**
```
gateway closed (1008): unauthorized: gateway token mismatch
Gateway target: ws://127.0.0.1:18789
Config: /home/admin/.clawdbot/clawdbot.json
Bind: loopback
```

**解决方案**
```bash
# 1. 统一令牌配置到所有位置
# 方案1：设置环境变量
export CLAWDBOT_GATEWAY_TOKEN=your_correct_token

# 方案2：在配置文件中统一令牌
cat > ~/.clawdbot/clawdbot.json << 'EOF'
{
  "gateway": {
    "auth": {
      "mode": "token",
      "token": "your_correct_token"
    },
    "remote": {
      "token": "your_correct_token"
    }
  }
}
EOF

# 2. 重启网关服务
clawdbot gateway stop
clawdbot gateway start

# 3. 验证连接
clawdbot gateway status
```

**关键要点**
- ✅ 确保令牌在所有配置位置保持一致
- ✅ 修改配置后必须重启服务
- ✅ 使用 `systemctl --user` 命令管理服务，不要使用 `pkill`

---

### 成功1：DingTalk图片发送修复
**成功步骤**
1. 识别API端点错误：从 `api.dingtalk.com/v1.0/media/upload` 改为 `oapi.dingtalk.com/media/upload`
2. 修正消息类型：从 `image` 改为 `sampleImageMsg`
3. 修正参数字段：从 `imageKey` 改为 `photoURL`
4. 逐步测试验证每个修改
5. 确认图片在钉钉中正常显示

**关键因素**
- 仔细阅读官方文档
- 逐步测试每个修改
- 保持API调用一致性
- 参考官方示例代码

**代码示例**
```javascript
// 正确的图片发送方式
const axios = require('axios');
const fs = require('fs');

async function sendDingTalkImage(webhookUrl, imagePath, accessToken) {
  try {
    // 1. 上传图片到钉钉服务器（使用旧版API）
    const uploadUrl = `https://oapi.dingtalk.com/media/upload?access_token=${accessToken}&type=image`;
    const formData = new FormData();
    formData.append('media', fs.createReadStream(imagePath));

    const uploadResponse = await axios.post(uploadUrl, formData, {
      headers: formData.getHeaders()
    });

    if (uploadResponse.data.errcode !== 0) {
      throw new Error(`Upload failed: ${uploadResponse.data.errmsg}`);
    }

    const media_id = uploadResponse.data.media_id;
    
    // 2. 发送图片消息（使用正确的 msgtype 和参数）
    const msg = {
      msgtype: "sampleImageMsg",  // 关键：使用 sampleImageMsg
      sampleImageMsg: {
        photoURL: media_id  // 关键：使用 photoURL 而非 imageKey
      }
    };
    
    await axios.post(webhookUrl, msg);
    console.log('Image sent successfully');
  } catch (error) {
    console.error('Error sending image:', error.message);
  }
}
```

---

### 成功2：网关令牌配置修复
**成功步骤**
1. 识别令牌配置分散问题（配置文件、环境变量、systemd服务文件）
2. 统一令牌配置到所有位置
3. 更新配置文件添加 `gateway.remote.token` 字段
4. 使用正确的systemd命令重启服务
5. 验证服务状态为active

**关键因素**
- 令牌配置在所有位置保持一致
- 使用systemd服务管理命令
- 重启后验证服务状态
- 避免使用 `pkill` 手动杀死进程

**命令示例**
```bash
# 1. 编辑配置文件
vi ~/.clawdbot/clawdbot.json

# 确保包含以下配置：
# {
#   "gateway": {
#     "auth": {
#       "mode": "token",
#       "token": "your_correct_token"
#     },
#     "remote": {
#       "token": "your_correct_token"  // 必须与 auth.token 一致
#     }
#   }
# }

# 2. 重启网关服务（使用正确的systemd命令）
clawdbot gateway stop
clawdbot gateway start

# 3. 验证服务状态
clawdbot gateway status

# 4. 查看服务日志
journalctl --user -u clawdbot-gateway -n 50
```

---

### 💡 重要技能和知识


### DingTalk API调用能力
**正确的API端点**
- 图片上传：`https://oapi.dingtalk.com/media/upload`
- 消息发送：`https://oapi.dingtalk.com/robot/send`
- 获取access_token：`https://oapi.dingtalk.com/gettoken`

**消息类型**
- 图片消息：`sampleImageMsg`（注意不是 `image`）
- Markdown消息：`markdown`
- 链接消息：`link`
- 文本消息：`text`

**关键参数**
- `sampleImageMsg` 需要 `photoURL` 字段（media_id值）
- 不要使用 `imageKey` 或 `sampleImage`

**配置管理**
```javascript
// Clawdbot配置示例
{
  "channels": {
    "dingtalk": {
      "enabled": true,
      "appKey": "your_app_key",
      "appSecret": "your_app_secret",
      "groupPolicy": "open",
      "dmPolicy": "open"
    }
  }
}
```

---

### 📊 失败原因分析


### 根本原因总结
| 失败类型 | 根本原因 | 频次 |
|---------|---------|------|
| API端点错误 | 混淆新版和旧版API端点 | 1次 |
| 消息类型错误 | 未仔细阅读官方文档的参数定义 | 2次 |
| 参数字段错误 | 参数名称拼写错误或使用了过时的字段名 | 1次 |
| 令牌配置错误 | 配置分散在多个位置且未同步更新 | 10+次 |
| 服务管理错误 | 使用手动进程杀死而非systemd命令 | 5次 |

---

### 🎯 最佳实践


### 1. DingTalk集成
1. **文档优先**
   - 始终从官方文档开始
   - 验证API版本兼容性
   - 参考官方示例代码

2. **渐进式测试**
   - 每次只修改一个问题
   - 修改后立即测试验证
   - 保留可回滚的备份

3. **参数验证**
   - 仔细检查参数名称拼写
   - 验证参数值格式
   - 使用正确的数据类型

### 2. 配置管理
1. **配置一致性**
   - 令牌和密钥在所有位置保持一致
   - 配置文件和环境变量同步
   - 使用版本控制管理配置变更

2. **服务管理**
   - 使用systemd命令管理服务
   - 避免手动杀死进程
   - 重启后验证服务状态

3. **日志监控**
   - 启用详细日志记录
   - 分析日志中的错误代码
   - 使用日志定位问题根源

---

*文档创建时间：2026-02-15*
*基于Clawdbot对话记录整理完成*