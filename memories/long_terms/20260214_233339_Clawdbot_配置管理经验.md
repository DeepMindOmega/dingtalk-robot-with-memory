# Clawdbot 配置管理经验

**Type**: long_term
**Source**: opencode_experience
**Created**: 2026-02-14T23:33:39.886708+00:00
**ID**: 126

**Tags**: config

---

## 概述
文档时间范围：2026-01-30 至 2026-02-13
系统：Clawdbot AI助手

### 失败1：令牌配置不一致导致连接失败
**问题描述**
- 浏览器功能无法使用
- CLI客户端连接被拒绝
- 错误信息：`gateway closed (1008): unauthorized: gateway token mismatch`

**失败原因**
1. **令牌配置分散在多个位置**：
   - 配置文件中：`gateway.auth.token`
   - 配置文件中：`gateway.remote.token`
   - 环境变量：`CLAWDBOT_GATEWAY_TOKEN`
   - systemd服务文件：`Environment=CLAWDBOT_GATEWAY_TOKEN`
   - 这些位置的令牌值不一致

2. **配置更新未同步**：
   - 修改了一个位置的令牌，但其他位置未更新
   - 服务仍在使用旧的令牌值

**错误日志**
```
gateway closed (1008): unauthorized: gateway token mismatch (provide gateway auth token)
Gateway target: ws://127.0.0.1:18789
Source: local loopback
Config: /home/admin/.clawdbot/clawdbot.json
```

**解决方案**
```bash
# 1. 统一令牌到所有位置
export CLAWDBOT_GATEWAY_TOKEN=your_correct_token

# 2. 在配置文件中统一令牌
cat > ~/.clawdbot/clawdbot.json << 'EOF'
{
  "gateway": {
    "port": 18789,
    "mode": "local",
    "bind": "loopback",
    "auth": {
      "mode": "token",
      "token": "your_correct_token"
    },
    "remote": {
      "token": "your_correct_token"
    },
    "tailscale": {
      "mode": "off",
      "resetOnExit": false
    }
  }
}
EOF

# 3. 更新systemd服务文件的环境变量
cat > ~/.config/systemd/user/clawdbot-gateway.service << 'EOF'
[Unit]
Description=Clawdbot Gateway Service
After=network.target

[Service]
Type=simple
ExecStart=/usr/bin/node /home/admin/.npm-global/lib/node_modules/clawdbot/dist/entry.js gateway --port 18789
Environment=CLAWDBOT_GATEWAY_PORT=18789
Environment=CLAWDBOT_GATEWAY_TOKEN=your_correct_token
Restart=on-failure
RestartSec=5

[Install]
WantedBy=default.target
EOF

# 4. 重新加载systemd配置
systemctl --user daemon-reload

# 5. 重启网关服务
clawdbot gateway stop
clawdbot gateway start

# 6. 验证服务状态
clawdbot gateway status
```

**关键要点**
- ✅ 确保令牌在所有配置位置保持一致（配置文件、环境变量、systemd服务文件）
- ✅ 修改配置后必须重新加载systemd配置并重启服务
- ✅ 使用systemd命令管理服务，不要使用pkill手动杀死进程

---

### 失败2：服务管理不当导致配置未生效
**问题描述**
- 修改配置文件后服务仍在使用旧配置
- 使用 `pkill -f clawdbot-gateway` 手动杀死进程
- 重启后服务仍使用旧的令牌配置

**失败原因**
1. **使用错误的服务管理方法**：
   - 使用 `pkill` 手动杀死进程
   - systemd服务管理器无法感知进程已被杀死
   - 新启动的进程可能不是通过systemd管理的

2. **未重新加载systemd配置**：
   - 修改systemd服务文件后未运行 `systemctl --user daemon-reload`
   - systemd仍在使用旧的服务单元文件

**错误日志**
```
Port 18789 is already in use.
- pid 1812 admin: clawdbot-gateway (127.0.0.1:18789)
- Gateway already running locally. Stop it (clawdbot gateway stop) or use a different port.
```

**解决方案**
```bash
# 错误做法（不要这样）：
pkill -f clawdbot-gateway

# 正确做法：
# 1. 使用systemd命令停止服务
systemctl --user stop clawdbot-gateway

# 2. 确认进程已停止
ps aux | grep clawdbot-gateway

# 3. 如果进程仍在运行，使用systemd强制停止
systemctl --user kill -s SIGKILL clawdbot-gateway

# 4. 修改配置文件
vi ~/.clawdbot/clawdbot.json

# 5. 修改systemd服务文件（如果需要）
vi ~/.config/systemd/user/clawdbot-gateway.service

# 6. 重新加载systemd配置
systemctl --user daemon-reload

# 7. 启动服务
systemctl --user start clawdbot-gateway

# 8. 验证服务状态
systemctl --user status clawdbot-gateway
clawdbot gateway status

# 9. 查看服务日志
journalctl --user -u clawdbot-gateway -n 50 -f
```

**关键要点**
- ✅ 始终使用systemd命令管理服务（stop/start/restart/status）
- ✅ 修改systemd服务文件后必须运行 `systemctl --user daemon-reload`
- ✅ 避免使用pkill手动杀死systemd管理的进程
- ✅ 使用 `journalctl` 查看服务日志而非普通日志文件

---

### 失败3：配置文件位置混淆
**问题描述**
- 多个配置文件同时存在
- 不确定哪个配置文件被实际使用
- 修改错误的配置文件导致问题持续存在

**失败原因**
1. **配置文件位置不明确**：
   - 主配置：`~/.clawdbot/clawdbot.json`
   - 其他可能的配置位置：`/etc/clawdbot/config.json`, 项目目录中的配置文件
   - 环境变量指定的配置路径

2. **配置优先级不清楚**：
   - 不清楚命令行参数、环境变量、配置文件的优先级
   - 可能使用了错误的配置文件

**解决方案**
```bash
# 1. 查找所有可能的配置文件
find ~ -name "clawdbot.json" 2>/dev/null
find /etc -name "clawdbot*" 2>/dev/null

# 2. 确认实际使用的配置文件路径
clawdbot gateway status
# 输出会显示 "Config (cli): ~/.clawdbot/clawdbot.json"

# 3. 查看环境变量
echo $CLAWDBOT_CONFIG

# 4. 检查systemd服务文件中的配置路径
cat ~/.config/systemd/user/clawdbot-gateway.service | grep Config

# 5. 确认唯一的配置文件位置
# 通常主配置文件位于：~/.clawdbot/clawdbot.json

# 6. 备份当前配置
cp ~/.clawdbot/clawdbot.json ~/.clawdbot/clawdbot.json.backup

# 7. 修改正确的配置文件
vi ~/.clawdbot/clawdbot.json

# 8. 验证JSON格式
jq '.' ~/.clawdbot/clawdbot.json
```

**关键要点**
- ✅ 主配置文件通常位于 `~/.clawdbot/clawdbot.json`
- ✅ 使用 `clawdbot gateway status` 确认实际使用的配置路径
- ✅ 修改配置前先备份
- ✅ 使用jq等工具验证JSON格式

---

### 成功1：统一配置管理
**成功步骤**
1. 识别所有配置位置（配置文件、环境变量、systemd服务文件）
2. 统一令牌配置到所有位置
3. 重新加载systemd配置
4. 使用正确的systemd命令重启服务
5. 验证服务状态和连接

**关键因素**
- 配置在所有位置保持一致
- 使用systemd命令管理服务
- 修改后重新加载配置
- 重启后验证状态

**配置示例**
```json
{
  "gateway": {
    "port": 18789,
    "mode": "local",
    "bind": "loopback",
    "auth": {
      "mode": "token",
      "token": "your_correct_token"
    },
    "remote": {
      "token": "your_correct_token"
    }
  }
}
```

---

### 成功2：systemd服务正确管理
**成功步骤**
1. 使用systemd命令停止服务
2. 修改配置文件
3. 修改systemd服务文件（如果需要）
4. 重新加载systemd配置
5. 启动服务并验证状态

**关键因素**
- 使用systemd命令而非pkill
- 修改服务文件后重新加载配置
- 启动后验证服务状态
- 使用journalctl查看日志

**命令示例**
```bash
# 标准服务管理流程
systemctl --user stop clawdbot-gateway
# 修改配置文件
systemctl --user daemon-reload
systemctl --user start clawdbot-gateway
systemctl --user status clawdbot-gateway
journalctl --user -u clawdbot-gateway -n 50
```

---

### 💡 重要技能和知识


### Clawdbot配置管理
**配置文件位置**
- 主配置：`~/.clawdbot/clawdbot.json`
- 环境变量：`CLAWDBOT_GATEWAY_TOKEN`, `CLAWDBOT_CONFIG`
- systemd服务：`~/.config/systemd/user/clawdbot-gateway.service`

**配置结构**
```json
{
  "gateway": {
    "port": 18789,
    "mode": "local",
    "bind": "loopback",
    "auth": {
      "mode": "token",
      "token": "your_auth_token_here"
    },
    "remote": {
      "token": "your_auth_token_here"  // 必须与auth.token一致
    },
    "tailscale": {
      "mode": "off",
      "resetOnExit": false
    }
  },
  "channels": {
    "dingtalk": {
      "enabled": true,
      "appKey": "your_dingtalk_app_key",
      "appSecret": "your_dingtalk_app_secret"
    }
  }
}
```

### 服务管理命令
```bash
# 查看服务状态
clawdbot gateway status
systemctl --user status clawdbot-gateway

# 启动服务
clawdbot gateway start
systemctl --user start clawdbot-gateway

# 停止服务
clawdbot gateway stop
systemctl --user stop clawdbot-gateway

# 重启服务
clawdbot gateway restart
systemctl --user restart clawdbot-gateway

# 查看服务日志
journalctl --user -u clawdbot-gateway -n 50
journalctl --user -u clawdbot-gateway -f  # 实时查看

# 重新加载systemd配置
systemctl --user daemon-reload

# 启用服务开机自启动
systemctl --user enable clawdbot-gateway

# 禁用服务开机自启动
systemctl --user disable clawdbot-gateway
```

---

### 📊 失败原因分析


### 根本原因总结
| 失败类型 | 根本原因 | 频次 |
|---------|---------|------|
| 令牌配置不一致 | 配置分散在多个位置且未同步更新 | 10+次 |
| 服务管理不当 | 使用pkill而非systemd命令 | 5次 |
| 配置未重新加载 | 修改systemd文件后未daemon-reload | 3次 |
| 配置文件位置混淆 | 不清楚实际使用的配置文件路径 | 2次 |
| JSON格式错误 | 配置文件语法错误 | 1次 |

---

### 🎯 最佳实践


### 1. 配置一致性
1. **统一配置源**
   - 将主配置作为单一事实来源
   - 确保所有配置位置与主配置一致
   - 使用版本控制管理配置变更

2. **配置验证**
   - 修改前备份原配置
   - 修改后验证JSON格式
   - 重启前确认配置正确

### 2. 服务管理
1. **使用systemd命令**
   - 始终使用systemctl命令管理服务
   - 避免使用pkill手动杀死进程
   - 修改服务文件后必须daemon-reload

2. **日志监控**
   - 使用journalctl查看服务日志
   - 实时监控服务状态
   - 定期检查服务健康状态

### 3. 问题排查
1. **配置问题排查**
   ```bash
   # 确认配置文件路径
   clawdbot gateway status

   # 验证JSON格式
   jq '.' ~/.clawdbot/clawdbot.json

   # 查看环境变量
   echo $CLAWDBOT_GATEWAY_TOKEN

   # 查找所有配置文件
   find ~ -name "clawdbot.json"
   ```

2. **服务问题排查**
   ```bash
   # 查看服务状态
   systemctl --user status clawdbot-gateway

   # 查看服务日志
   journalctl --user -u clawdbot-gateway -n 100

   # 检查进程
   ps aux | grep clawdbot

   # 检查端口占用
   netstat -tuln | grep 18789
   ```

---

### 🔧 常用命令速查


### 配置管理
```bash
# 备份配置
cp ~/.clawdbot/clawdbot.json ~/.clawdbot/clawdbot.json.backup

# 编辑配置
vi ~/.clawdbot/clawdbot.json

# 验证JSON格式
jq '.' ~/.clawdbot/clawdbot.json

# 查看当前配置
cat ~/.clawdbot/clawdbot.json | jq

# 修改令牌（示例）
jq '.gateway.auth.token = "new_token"' ~/.clawdbot/clawdbot.json > /tmp/config.json
mv /tmp/config.json ~/.clawdbot/clawdbot.json
```

### 服务管理
```bash
# 重启服务（标准流程）
clawdbot gateway restart
# 等价于：
systemctl --user restart clawdbot-gateway

# 查看详细状态
clawdbot gateway status
systemctl --user status clawdbot-gateway -l

# 查看日志
journalctl --user -u clawdbot-gateway -n 50
journalctl --user -u clawdbot-gateway --since "5 minutes ago"
journalctl --user -u clawdbot-gateway -f  # 实时跟踪
```

---

*文档创建时间：2026-02-15*
*基于Clawdbot对话记录整理完成*